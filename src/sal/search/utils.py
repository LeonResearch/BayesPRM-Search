...
def single_request(instruction, client, config):
...
def add_chat_template(messages, chat_template):
    # Define context variables for template rendering
    context = {
        'custom_tools': [{"name": "weather_tool", "parameters": {"location": "New York"}}],
        'tools_in_user_message': True,
        'messages': messages,
        'strftime_now': datetime.now,
        'add_generation_prompt': False
    }

    # Initialize the Jinja2 environment and render the template
    template = jinja2.Template(chat_template)
    formatted_messages = template.render(context)
