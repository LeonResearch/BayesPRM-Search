...
from sal.search.utils import single_request


def best_of_n(x, config: Config, prm: PRM, clients: list):
    convs = [
        [
    ...
    client_ids = [i for i in x['client_id']]
    duplicated_convs = [c for conv in convs for c in [conv] * config.n]
    duplicated_client_ids = [idx for idx in client_ids * config.n]

    # vLLM serving by sending batched requests with Pool
    responses = []
    with ThreadPoolExecutor(max_workers=len(convs)) as executor:
        for idx, instruction in enumerate(duplicated_convs):
            single_response = executor.submit(
                single_request, 
                instruction, 
                clients[duplicated_client_ids[idx]],
                config,
            )
            responses.append(single_response)
        responses = [i.result() for i in responses] # Extract the results from futures
    ...
    for i in range(len(completions)):
        completions[i] = [
            output
            for output in responses[i * config.n : (i + 1) * config.n]
        ]
